...
def cut(obj, sec):
    ...
    #     pass
    # else:
    #     result = [obj[i:i+sec+remanent_count]
...
def get_feature_packet(label_pcap, payload_len: int) -> t.List:
    feature_data = []
    ...
    for packet in packets:
        ...
        new_packet_string = packet_string[76:]  # 原始pcap 去掉前76bytes 每2个16进制合1
...
def get_feature_flow(label_pcap, payload_len, payload_pac):
    ...
    if len(feature_result) == 0:
        ...
        if (
            len(feature_result[label_pcap, extract_keys[1],
                extract_keys[2]].ip_lengths)
            < 3
        ):
            print(
    ...
    for key in feature_result.keys():
        ...
        # 类型为 dict[, (逗号分隔的tls content_type, frame编号)]
        ...
        extension_string_dict = {k: ",".join(v)
    ...
    packet_direction = [1 if length > 0 else -1 for length in packet_length[0]]

    packet_index = 0
    for packet in packets[:payload_pac]:
        packet_data = packet.copy()
        data = binascii.hexlify(bytes(packet_data))
...
def generation(
    ...
    for dirs in os.listdir(pcap_path):
        label_name_list.append(dirs)
        ...
        for p, dd, ff in os.walk(os.path.join(pcap_path, dirs)):
            ...
            if splitcap:
                ...
                session_pcap_path[dirs] = pcap_path + "/splitcap/" + dirs
            else:
                ...
                for p, d, f in os.walk(session_pcap_path[key]):
                    for file in f:
                        ...
                        if not os.path.getsize(current_file):
                            ...
                            print("current pcap %s is 0KB and delete" %
                                  current_file)
                        else:
                            current_packet: scapy.PacketList = scapy.rdpcap(
                                p + "/" + file
                            )
                            file_size = float(
        ...
        for r_f in r_files:
            if dataset_level == "flow":
                ...
                feature_data = get_feature_packet(
                    r_f, payload_len=payload_length)
        ...
        for label in json_data.keys():
            ...
            if samples[label_count] < sample_num:
                ...
                for sample_index in random.sample(
                    list(json_data[label]
                         [features[feature_index]].keys()), 1500
                ):
                    x_label.append(
