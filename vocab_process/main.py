...
word_dir = "./corpora/"
word_name = "encrypted_traffic_burst2.txt"
...
def pcap_preprocess():
    ...
    end_date = tls_date[1]
    packet_num = 0
    for d in os.scandir('/mnt/dataset/ET-BERT/datasets/pcap'):
...
def preprocess(pcap_dir):
    ...
    for parent, dirs, files in os.walk(pcap_dir):
        for file in files[:2]:
            if "pcapng" not in file and "pkl" not in file:
                n += 1
...
def build_BPE():
    ...
    # trainer = trainers.BpeTrainer(vocab_size=65536, min_frequency=2)
    tokenizer.train([path, path], trainer=trainer)
    ...
    for sub_string_index in range(len(sentence)):
        if sub_string_index != (len(sentence) - 1):
            ...
            if token_count > 256:
                ...
                merge_word_bigram = sentence[sub_string_index] + \
